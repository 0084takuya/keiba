...
class DynamicsNet(nn.Module):
    ...
    def forward(self, state, action):
        ...
        x = torch.cat([state, action], dim=0)  # [hidden_dim+1]
        return self.relu(self.fc(x))
...
if __name__ == '__main__':
    validate_pythonpath('muzero_keiba_simple.py')
    target_days = parse_days_arg(sys.argv, 'muzero_keiba_simple.py')
    start_time = time.time()
    print('=== MuZeroモデル 実行開始 ===')
    ...
    acc, f1, recall, gpt_advice = train_muzero(params, run_dir)
    end_time = time.time()
    print('=== MuZeroモデル 実行終了 ===')
