    ...
    for i, (train_idx, test_idx) in enumerate(kf.split(rows)):
        ...
        for row in test_rows:
            try:
                ...
                all_X.append([feature_dict.get(f.name, 0) for f in FEATURES])
                chaku = int(row[4]) if len(row) > 4 else 0
...
def train():
    ...
    # SMOTEで増えた分のカテゴリindex列も同じインデックスで増やす
    n_orig = len(X_train_cont)
    n_res = len(X_train_cont_res)
    if n_res > n_orig:
        # SMOTEのサンプル生成時、元データのインデックスがresample_indicesに格納される
        resample_indices = smote.sample_indices_ if hasattr(smote, 'sample_indices_') else np.arange(n_res)
        X_train_cat_res = X_train_cat[resample_indices]
    else:
