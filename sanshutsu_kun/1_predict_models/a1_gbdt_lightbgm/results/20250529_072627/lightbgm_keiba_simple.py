...
def fetch_data_time_split(test_ratio=0.2):
    ...
    for idx, row in df.iterrows():
        ...
        feats = {
            ...
            # 'target': row['target'],  # ←未来情報なので除外
            # 'RACE_CODE': row['RACE_CODE']  # ←未来情報なので除外
        }
    ...
    return X_train, X_test, y_train, y_test, n0, n1
...
def main():
    ...
    X_train, X_test, y_train, y_test, n0, n1 = fetch_data_time_split()
    if len(X_train) == 0:
        print('データがありません')
        return
    # GroupKFoldでRACE_CODE単位で分割
    groups = df.loc[X_train.index, 'RACE_CODE'].reset_index(drop=True)
