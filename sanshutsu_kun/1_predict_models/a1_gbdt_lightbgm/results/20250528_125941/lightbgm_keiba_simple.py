...
def fetch_data():
    ...
    for col in ['ZEN_SO_CHAKUJUN','ZEN_SO_NINKI','ZEN_SO_BATAIJU','ZEN_SO_BATAIJU_DIFF','SHUSSO_KAISU','FUTAN_JURYO_DIFF']:
        df[col] = pd.to_numeric(df[col], errors='coerce')
    # --- コース適性・距離適性特徴量 ---
    # コースID（RACE_CODEの先頭6桁: 開催年+場コード+回次）
    df['COURSE_ID'] = df['RACE_CODE'].astype(str).str[:6]
    # 距離を100m単位で丸める
    df['KYORI_ROUND'] = (df['KYORI'] // 100) * 100
    # 馬ごとに同コース・同距離での3着率・着順平均
    course_3in = df.groupby(['KETTO_TOROKU_BANGO','COURSE_ID'])['target'].transform('mean')
    course_chakujun = df.groupby(['KETTO_TOROKU_BANGO','COURSE_ID'])['ZEN_SO_CHAKUJUN'].transform('mean')
    kyori_3in = df.groupby(['KETTO_TOROKU_BANGO','KYORI_ROUND'])['target'].transform('mean')
    kyori_chakujun = df.groupby(['KETTO_TOROKU_BANGO','KYORI_ROUND'])['ZEN_SO_CHAKUJUN'].transform('mean')
    df['COURSE_3IN_RATE'] = course_3in
    df['COURSE_CHAKUJUN_MEAN'] = course_chakujun
    df['KYORI_3IN_RATE'] = kyori_3in
    df['KYORI_CHAKUJUN_MEAN'] = kyori_chakujun
    ...
    def rolling_slope(arr):
        ...
        return m
    df['ZEN_5R_CHAKUJUN_SLOPE'] = df.groupby('KETTO_TOROKU_BANGO')['ZEN_SO_CHAKUJUN'].rolling(5, min_periods=2).apply(rolling_slope, raw=True).reset_index(0,drop=True).fillna(0)
    df['ZEN_5R_BATAIJU_SLOPE'] = df.groupby('KETTO_TOROKU_BANGO')['ZEN_SO_BATAIJU'].rolling(5, min_periods=2).apply(rolling_slope, raw=True).reset_index(0,drop=True).fillna(0)

    # --- 特徴量リスト（未来情報・ID・ラベル直結情報を徹底除外） ---
    feature_cols = [
        'BAREI',                # 馬齢
        'SEIBETSU_CODE',        # 性別
        'BATAIJU',              # 馬体重
        'BATAIJU_DIFF',         # 馬体重変動（過去→現在）
        'FUTAN_JURYO',          # 斤量
        'FUTAN_JURYO_DIFF',     # 斤量変動
        'KISHU_CODE',           # 騎手コード
        'CHOKYOSHI_CODE',       # 調教師コード
        'KYORI',                # 距離
        'TRACK_CODE',           # コース種別
        'SHUSSO_TOSU',          # 頭数
        'KISHU_3IN_RATE',       # 騎手3着率
        'CHOKYOSHI_3IN_RATE',   # 調教師3着率
        'UMA_5R_3IN_RATE',      # 馬の過去5走3着率
        'RACE_KANKAKU',         # レース間隔
        'SHUSSO_KAISU',         # 出走回数
        'TENKO_JOTAI_GENZAI',   # 天候
        'BABA_JOTAI_SHIBA_GENZAI', # 芝馬場状態
        'BABA_JOTAI_DIRT_GENZAI',  # ダート馬場状態
        'KISHU_CHOKYOSHI_PAIR_3IN_RATE', # 騎手調教師ペア3着率
        'ZEN_5R_CHAKUJUN_MEAN', # 過去5走着順平均
        'ZEN_5R_CHAKUJUN_DIFF', # 過去5走着順変化率
        'COURSE_3IN_RATE',      # コース適性（過去同コース3着率）
        'COURSE_CHAKUJUN_MEAN', # コース適性（過去同コース着順平均）
        'KYORI_3IN_RATE',       # 距離適性（過去同距離3着率）
        'KYORI_CHAKUJUN_MEAN',  # 距離適性（過去同距離着順平均）
        'ZEN_5R_CHAKUJUN_SLOPE',# 過去5走着順トレンド
        'ZEN_5R_BATAIJU_SLOPE'  # 過去5走馬体重トレンド
    ]
    X = df[feature_cols]
    y = df['target']
    end_time = time.time()
