...
def fetch_data():
    ...
    # --- メインデータ ---
    print('メインデータ取得開始')
    query = '''SELECT BAREI, SEIBETSU_CODE, BATAIJU, FUTAN_JURYO, TANSHO_NINKIJUN, KISHU_CODE, CHOKYOSHI_CODE, KETTO_TOROKU_BANGO, RACE_CODE, KAISAI_NEN, KAISAI_GAPPI, KAKUTEI_CHAKUJUN, WAKUBAN FROM umagoto_race_joho WHERE CONCAT(KAISAI_NEN, LPAD(KAISAI_GAPPI, 4, '0')) >= %s AND KISHU_CODE IS NOT NULL AND RACE_CODE IS NOT NULL AND KETTO_TOROKU_BANGO IS NOT NULL AND CHOKYOSHI_CODE IS NOT NULL AND KAISAI_NEN IS NOT NULL AND KAISAI_GAPPI IS NOT NULL AND WAKUBAN IS NOT NULL'''
    cursor.execute(query, (date_min,))
    rows = cursor.fetchall()
    columns = ['BAREI','SEIBETSU_CODE','BATAIJU','FUTAN_JURYO','TANSHO_NINKIJUN','KISHU_CODE','CHOKYOSHI_CODE','KETTO_TOROKU_BANGO','RACE_CODE','KAISAI_NEN','KAISAI_GAPPI','KAKUTEI_CHAKUJUN','WAKUBAN']
    ...
    for idx, row in df.iterrows():
        ...
        feats['RACE_CODE'] = row['RACE_CODE']  # 必ずRACE_CODEを含める
        feature_rows.append(feats)
    feature_df = pd.DataFrame(feature_rows)
    # カテゴリ変数をLabelEncoder（WAKUBANも追加）
    for col in ['SEIBETSU_CODE','KISHU_CODE','CHOKYOSHI_CODE','TENKO_JOTAI_GENZAI','BABA_JOTAI_SHIBA_GENZAI','BABA_JOTAI_DIRT_GENZAI','WAKUBAN']:
    ...
    feature_cols = [
        ...
        'KISHU_CODE','CHOKYOSHI_CODE','WAKUBAN','KYORI','TRACK_CODE','SHUSSO_TOSU','KISHU_3IN_RATE','CHOKYOSHI_3IN_RATE','UMA_5R_3IN_RATE','RACE_KANKAKU','SHUSSO_KAISU','TENKO_JOTAI_GENZAI','BABA_JOTAI_SHIBA_GENZAI','BABA_JOTAI_DIRT_GENZAI',
...
def main():
    ...
    smote = SMOTE(sampling_strategy=0.5, random_state=42)  # 正例が全体の50%になるまで水増し
    ...
    # --- CatBoost学習 ---
    cat_model = CatBoostClassifier(iterations=200, depth=7, learning_rate=0.03, scale_pos_weight=n0/n1 if n1>0 else 1.0, l2_leaf_reg=1.0, verbose=0, random_seed=42, class_weights=[1, n0/n1 if n1>0 else 1.0])
...
競馬の3着以内に入る馬の特徴をLightGBM+XGBoost+CatBoostアンサンブルで学習したモデルの評価結果です。\n\n最適閾値: {best_threshold_f1:.4f}\n精度: {acc:.4f}\nF1スコア: {f1:.4f}\n詳細:\n{classification_report(y_test, preds)}\n\n以下のグラフ画像（base64エンコード済み）も参考にして、モデル改善のためのアドバイスを日本語で簡潔に出力してください。\n"""
    ...
    cat_model_sel = CatBoostClassifier(iterations=200, depth=7, learning_rate=0.03, scale_pos_weight=n0/n1 if n1>0 else 1.0, l2_leaf_reg=1.0, verbose=0, random_seed=42, class_weights=[1, n0/n1 if n1>0 else 1.0])
