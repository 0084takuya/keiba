...
def fetch_data():
    ...
    odds_tansho = {(row[0], row[1]): (float(row[2]) if row[2] else 0, int(row[3]) if row[3] else 0) for row in cursor.fetchall()}
    # --- odds1_fukusho ---
    print('odds1_fukusho取得開始')
    cursor.execute('SELECT RACE_CODE, UMABAN, ODDS_SAITEI, ODDS_SAIKOU, NINKI FROM odds1_fukusho')
    odds_fukusho = {(row[0], row[1]): (float(row[2]) if row[2] else 0, float(row[3]) if row[3] else 0, int(row[4]) if row[4] else 0) for row in cursor.fetchall()}
    # --- keito_joho2（血統） ---
    ...
    bataiju_dict = {(row[0], row[1]): float(row[2]) if row[2] else 0 for row in cursor.fetchall()}
    # --- kishu_master（騎手成績） ---
    ...
    # カテゴリ変数をLabelEncoder
    for col in ['SEIBETSU_CODE','KISHU_CODE','CHOKYOSHI_CODE','WAKUBAN','KEITO_ID','KEITO_MEI']:
