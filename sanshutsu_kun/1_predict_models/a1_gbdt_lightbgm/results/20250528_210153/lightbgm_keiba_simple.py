...
def main():
    ...
    # --- CatBoost学習 ---
    cat_model = CatBoostClassifier(iterations=200, depth=7, learning_rate=0.03, l2_leaf_reg=1.0, verbose=0, random_seed=42, class_weights=[1, n0/n1 if n1>0 else 1.0])
...
競馬の3着以内に入る馬の特徴をLightGBM+XGBoost+CatBoostアンサンブルで学習したモデルの評価結果です。\n\n最適閾値: {best_threshold_f1:.4f}\n精度: {acc:.4f}\nF1スコア: {f1:.4f}\n詳細:\n{classification_report(y_test, preds)}\n\n以下のグラフ画像（base64エンコード済み）も参考にして、モデル改善のためのアドバイスを日本語で簡潔に出力してください。\n"""
    ...
    cat_model_sel = CatBoostClassifier(iterations=200, depth=7, learning_rate=0.03, l2_leaf_reg=1.0, verbose=0, random_seed=42, class_weights=[1, n0/n1 if n1>0 else 1.0])
