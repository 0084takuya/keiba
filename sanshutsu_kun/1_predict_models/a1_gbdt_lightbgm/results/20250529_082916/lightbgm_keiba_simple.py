...
def fetch_data_time_split(test_ratio=0.2):
    ...
    for uma, group in df.groupby('KETTO_TOROKU_BANGO'):
        ...
        for i, row in group.iterrows():
            idx = row['index']
            if i > 0:
                if prev_bataiju and prev_bataiju != 0:
                    df.loc[idx, 'BATAIJU_DIFF_RATE'] = (row['BATAIJU'] - prev_bataiju) / prev_bataiju
                if prev_ninki and prev_ninki != 0:
                    df.loc[idx, 'NINKI_DIFF_RATE'] = (row['TANSHO_NINKIJUN'] - prev_ninki) / prev_ninki
            prev_bataiju = row['BATAIJU']
            prev_ninki = row['TANSHO_NINKIJUN']
    # --- Target Encoding ---
