...
def fetch_data():
    ...
    feature_cols = [
        ...
        'COURSE_3IN_RATE','COURSE_CHAKUJUN_MEAN','KYORI_3IN_RATE','KYORI_CHAKUJUN_MEAN',
        'ZEN_5R_CHAKUJUN_SLOPE','ZEN_5R_BATAIJU_SLOPE'
    ]
    X = df[feature_cols]
    y = df['target']
    end_time = time.time()
...
競馬の3着以内に入る馬の特徴をLightGBM+XGBoost+CatBoostアンサンブルで学習したモデルの評価結果です。\n\n最適閾値: {best_threshold:.4f}\n精度: {acc:.4f}\nF1スコア: {f1:.4f}\n詳細:\n{report}\n\n以下のグラフ画像（base64エンコード済み）も参考にして、モデル改善のためのアドバイスを日本語で簡潔に出力してください。\n"""
    ...
    for feat in top10_features:
        ...
        if jp_font:
            plt.title(f'特徴量[{feat}]の分布（全体/FN/FP）', fontproperties=jp_font)
        else:
            plt.title(f'特徴量[{feat}]の分布（全体/FN/FP）')
        plt.legend()
        plt.tight_layout()
        if jp_font:
            plt.savefig(os.path.join(run_dir, f'feature_dist_{feat}.png'), bbox_inches='tight', dpi=120, fontproperties=jp_font)
        else:
