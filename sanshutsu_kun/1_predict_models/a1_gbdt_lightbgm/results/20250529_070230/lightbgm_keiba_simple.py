...
def fetch_data():
    ...
    cursor.execute('SELECT RACE_CODE, KYORI, BABA_CODE, BABA_JOTAI_CODE, TENKO_CODE, COURSE_CODE FROM race_shosai')
    race_shosai_dict = {}
    for row in cursor.fetchall():
        race_shosai_dict[row[0]] = {
            'KYORI': float(row[1]) if row[1] else 0,
            'BABA_CODE': row[2] or '',
            'BABA_JOTAI_CODE': row[3] or '',
            'TENKO_CODE': row[4] or '',
            'COURSE_CODE': row[5] or ''
        }
    # --- 前走情報（自己JOIN） ---
    print('前走情報JOIN開始')
    ...
    for idx, row in df.iterrows():
        ...
        # 馬場・コース・距離・天候
        race_shosai = race_shosai_dict.get(race_code, {'KYORI':0,'BABA_CODE':'','BABA_JOTAI_CODE':'','TENKO_CODE':'','COURSE_CODE':''})
        # 既存特徴量＋追加
        feats = {
            'BAREI': row['BAREI'],
            ...
            'BABA_CODE': race_shosai['BABA_CODE'],
            'BABA_JOTAI_CODE': race_shosai['BABA_JOTAI_CODE'],
            'TENKO_CODE': race_shosai['TENKO_CODE'],
            'COURSE_CODE': race_shosai['COURSE_CODE'],
    ...
    # カテゴリ変数をLabelEncoder
    for col in ['SEIBETSU_CODE','KISHU_CODE','CHOKYOSHI_CODE','WAKUBAN','KEITO_ID','KEITO_MEI','BABA_CODE','BABA_JOTAI_CODE','TENKO_CODE','COURSE_CODE','prev_kishu','prev_course']:
    ...
    feature_cols = [
        ...
        'KYORI','BABA_CODE','BABA_JOTAI_CODE','TENKO_CODE','COURSE_CODE',
