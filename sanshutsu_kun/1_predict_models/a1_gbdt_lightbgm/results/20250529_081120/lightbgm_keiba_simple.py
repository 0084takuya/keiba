...
def fetch_data_time_split(test_ratio=0.2):
    ...
    # --- 特徴量リスト ---
    features = [
        'BAREI', 'SEIBETSU_CODE', 'BATAIJU', 'BATAIJU_DIFF', 'FUTAN_JURYO', 'TANSHO_NINKIJUN',
        'BABA_CODE', 'KAKUSITU_CODE', 'chaku3mean', 'ninki3mean',
        'KISHU_CODE_te', 'CHOKYOSHI_CODE_te', 'COURSE_CODE_te', 'WAKUBAN_te',
        'KISHU_CODE_3chaku_mean', 'CHOKYOSHI_CODE_3chaku_mean'
    ]
    features = [f for f in features if f in df.columns]
    # 型変換
    for col in ['BAREI', 'SEIBETSU_CODE', 'FUTAN_JURYO', 'TANSHO_NINKIJUN']:
        if col in df.columns:
...
def main():
    ...
    if len(X_train) == 0:
        ...
        return
    # --- SMOTEによるオーバーサンプリング ---
    print(f'オーバーサンプリング前: {np.bincount(y_train)}')
    smote = SMOTE(sampling_strategy=1.0, random_state=42)
    X_train, y_train = smote.fit_resample(X_train, y_train)
    print(f'SMOTE後: {np.bincount(y_train)}')
    ...
    if len(recall_idx) > 0:
        best_idx_recall = recall_idx[-1]  # 最小閾値でrecall>=0.85
        best_threshold_recall = thresholds[best_idx_recall] if best_idx_recall < len(thresholds) else 0.5
        print(f'Recall>={recall_target}となる最小閾値: {best_threshold_recall:.3f}')
        preds_recall = (lgb_probs >= best_threshold_recall).astype(int)
        ...
        print(f'--- Recall重視評価（閾値={best_threshold_recall:.3f}） ---')
        ...
        # グラフ・GPTアドバイス・保存も同様に実施
        graph_paths = plot_and_save_graphs(y_test, preds_recall, lgb_probs, train_losses, train_accuracies, run_dir)
        # 画像base64化
        images_b64 = []
