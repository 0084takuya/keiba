...
def main():
    ...
    for th, label in zip([0.5, best_threshold_f1], ['0.5','F1最大']):
        ...
        # --- グラフ出力 ---
        graph_paths = plot_and_save_graphs(y_test, preds, lgb_probs, train_losses, train_accuracies, run_dir)
        # 画像base64化
        images_b64 = []
...
競馬の3着以内に入る馬の特徴をLightGBMで学習したモデルの評価結果です。\n\n最適閾値: {th:.4f}\n精度: {acc:.4f}\nF1スコア: {f1:.4f}\n詳細:\n{classification_report(y_test, preds)}\n\n"""
        ...
        # --- 結果保存 ---
        with open(os.path.join(run_dir, f'eval_and_gpt_advice_{label}.txt'), 'w', encoding='utf-8') as f:
            f.write('--- 評価結果 ---\n')
