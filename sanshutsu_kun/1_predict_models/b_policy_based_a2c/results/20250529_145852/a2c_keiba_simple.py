    ...
    def make_features(df, ref_df):
        ...
        for idx, (_, row) in enumerate(df.iterrows()):
            ...
            if idx > 0 and row['KETTO_TOROKU_BANGO'] == df.iloc[idx-1]['KETTO_TOROKU_BANGO']:
                feature_dict['BATAIJU_DIFF'] = float(row['BATAIJU']) - float(df.iloc[idx-1]['BATAIJU'])
            else:
                feature_dict['BATAIJU_DIFF'] = 0
            # 馬体重×距離
            bataiju = float(row['BATAIJU']) if not pd.isnull(row['BATAIJU']) else 0
            kyori = float(row['KYORI']) if not pd.isnull(row['KYORI']) else 0
            feature_dict['BATAIJU_KYORI'] = bataiju * kyori
            # 馬齢×負担重量
            barei = float(row['BAREI']) if not pd.isnull(row['BAREI']) else 0
            futan = float(row['FUTAN_JURYO']) if not pd.isnull(row['FUTAN_JURYO']) else 0
