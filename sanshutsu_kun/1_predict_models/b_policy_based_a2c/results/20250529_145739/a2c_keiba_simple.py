    ...
    def make_features(df, ref_df):
        ...
        for idx, (_, row) in enumerate(df.iterrows()):
            ...
            if idx > 0 and row['KETTO_TOROKU_BANGO'] == df.iloc[idx-1]['KETTO_TOROKU_BANGO']:
                feature_dict['BATAIJU_DIFF'] = row['BATAIJU'] - df.iloc[idx-1]['BATAIJU']
            else:
                feature_dict['BATAIJU_DIFF'] = 0
            # 馬体重×距離
            feature_dict['BATAIJU_KYORI'] = row['BATAIJU'] * row['KYORI']
            # 馬齢×負担重量
            feature_dict['BAREI_FUTAN_JURYO'] = row['BAREI'] * row['FUTAN_JURYO']
            # レース間隔（日数）
            if idx > 0 and row['KETTO_TOROKU_BANGO'] == df.iloc[idx-1]['KETTO_TOROKU_BANGO']:
